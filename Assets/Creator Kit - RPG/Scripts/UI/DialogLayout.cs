...
namespace RPGM.UI
{
    ...
    public class DialogLayout : MonoBehaviour
    {
        ...
        //this positions the sprite.
        //the text inside vector 3 initially said 1, 0, 0. changing it made the chicken go up instead of to the right.
        // 1, 0, 0 chicken on right
        // 0, 1, 0 chicken in middle
        // -1, 1, 0 chicken top left
        //this is the old correct one
        ...
        void PositionIcon()
        {
            if (iconRenderer.sprite != null)
            {
                ...
                // p.x -= iconRenderer.size.x * 1f;
                iconRenderer.transform.localPosition = p;
            }
        }
        //this is the end of the old correct one
        /*
        ...
        void AlignIcon()
        {
            if (iconRenderer.sprite != null)
                switch (theAlignment)
                {
                    case iconAlignmnent.Left:
                        var p = new Vector3(-1, 1, 0);
                        iconRenderer.transform.localPosition = p;
                        break;
                    case iconAlignmnent.Center:
                        var p2 = new Vector3(0, 1, 0);
                        iconRenderer.transform.localPosition = p2;
                        break;
                    case iconAlignmnent.Right:
                        var p3 = new Vector3(1, 0, 0);
                        iconRenderer.transform.localPosition = p3;
                        break;
                    default:
                        var p4 = new Vector3(-1, 1, 0);
                        iconRenderer.transform.localPosition = p4;
                        break;
                }
        }
        */
